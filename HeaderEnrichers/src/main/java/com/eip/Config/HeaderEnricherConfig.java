...
public class HeaderEnricherConfig {
    ...
    @Bean
    public MessageChannel headerEnricherAddAuthChannel(){
        return new DirectChannel();
    }

    @Bean
    public MessageChannel headerEnricherPartyOwnerChannel(){
        return new DirectChannel();
    }
    ...
    @Bean
    public MessageChannel headerEnricherRemoveAuthChannel(){
        return  new DirectChannel();
    }
    ...
    public HeaderEnricher headerEnricherAuthToken(){
        return new HeaderEnricher(Collections.singletonMap("AUTH_TOKEN",
                                    new StaticHeaderValueMessageProcessor<>("12345")));
    }

    @Bean
    @Transformer(inputChannel = "headerEnricherPartyOwnerChannel" ,outputChannel = "auditChannel")
    public HeaderEnricher headerEnricherParty(){
        ...
        //Return a new HeaderEnricher wiht a Spel header
        return new HeaderEnricher(Collections.singletonMap("PARTY_OWNER",
